import path from 'node:path';
import * as grpc from '@grpc/grpc-js';
import * as protoLoader from '@grpc/proto-loader';
import { CONFIG } from '../lib/config.js';

// Use path relative to repo root (works for both runtime and test runs from backend/ dir)
const PROTO_PATH = path.resolve(process.cwd(), '../contracts/video/video_service.proto');

type Promisify<T> = {
  [K in keyof T]: T[K] extends (...args: infer A) => any
    ? (...args: A extends [...infer P, any] ? P : A) => Promise<any>
    : never;
};

export function createVideoClient() {
  const packageDefinition = protoLoader.loadSync(PROTO_PATH, {
    keepCase: false,
    longs: String,
    enums: String,
    defaults: true,
    oneofs: true,
  });
  const proto = grpc.loadPackageDefinition(packageDefinition) as any;
  const VideoService = proto.video.VideoService as any;
  const client = new VideoService(CONFIG.videoTarget, grpc.credentials.createInsecure());
  return client as any as {
    EnableOptionalElement: (
      req: { element: string },
      cb: (err: grpc.ServiceError | null, res: {}) => void
    ) => void;
    DisableOptionalElement: (
      req: { element: string },
      cb: (err: grpc.ServiceError | null, res: {}) => void
    ) => void;
  };
}
